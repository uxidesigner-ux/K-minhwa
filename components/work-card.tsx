...
type WorkCardProps = { href: string; image: string; index: number; title: string; year: string; medium: string; className?: string };
const MAX_SHIFT = 8;

export function WorkCard({ href, image, index, title, year, medium, className = '' }: WorkCardProps) {
  ...
  return <Link href={href} className={`work-card ${className}`} onPointerMove={updatePosition} onPointerLeave={resetPosition}>
