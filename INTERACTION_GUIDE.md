...
- Typography: H1 is the visual event. Home English is a single Fraunces word (`Remember` as `Re` + italic `mem` + `ber`). Home Korean is a single Noto Serif KR word (`기억`) with no italic. H2 is a quieter editorial anchor; mono labels provide cataloguing. Keep body measure under ~40rem.
