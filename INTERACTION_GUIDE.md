...
- Cursor: default cursor stays quiet. A desktop-only liquid-glass ring (65px idle / 91px interactive, ≈120% of the prior size) may appear over links and media, with 80–120ms lag and a filled warm-white center dot; it never obscures artwork. Omit on touch.
...
- Header: persistent, compact, transparent over hero and black elsewhere. Work/Artist are the only primary destinations. Admin is not public navigation. The home header splits into two inset pills: mark + menu on the left, locale toggle on the right. Home wordmark is `雲住`.
...
- ENTER ARCHIVE: hover increases letter-spacing and lengthens the arrow (gap + scaleX); reduced-motion keeps the resting metrics.
...
- The custom cursor is a liquid-glass 65px ring with a filled center dot and a 91px interactive state. It replaces the native cursor only for fine pointers, and is absent for touch, mobile, and reduced-motion users.
